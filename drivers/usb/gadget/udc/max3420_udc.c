...
static int max3421_start(struct max3420_udc *udc)
{
	unsigned long flags;
	int todo;
...
static irqreturn_t max3420_xirq10_handler(int irq, void *dev_id)
{
...
static int max3420_handle_irqs(struct max3420_udc *udc)
{
	...
	if (usbirq & bURESDNIRQ) {
		...
		spi_wr8(udc, MAX3420_REG_EPIEN, bSUDAVIRQ | bIN0BAVIRQ | bOUT0DAVIRQ);
		return 1;
	}
...
{
	unsigned maxp = usb_endpoint_maxp(desc);
	unsigned long flags;
...
static int max3420_probe(struct spi_device *spi)
{
	...
	err = devm_request_irq(&spi->dev, spi->irq, max3420_irq_handler, 0,
			       "max3420", udc);
	if (err < 0) {
	...
	if (!udc->is_selfpowered) {
		int xirq10 = of_irq_get(spi->dev.of_node, 1);
		err = devm_request_irq(&spi->dev, xirq10, max3420_xirq10_handler, 0,
				       "xirq10", udc);
		if (err < 0) {
			dev_err(&spi->dev, "failed to request irq %d\n", xirq10);
			return err;
...
static const struct of_device_id max3420_udc_spi_of_match[] = {
...
static struct spi_driver max3420_driver = {
	.driver = {
		.name = "max3421_udc",
		.of_match_table = of_match_ptr(max3420_udc_spi_of_match),
	},
	.probe = max3420_probe,
